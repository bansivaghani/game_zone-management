...
include("code/db_connection.php"); // Include database connection

// Set the number of records per page
$records_per_page = 10;

// Find out how many total records are in the database
$sql_count = "SELECT COUNT(*) AS total FROM `payment` where action='cancelled'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (in_array($new_action, ['pending', 'approved', 'cancelled'])) {
        $sql = "UPDATE `payment` SET `cancelled` = ? WHERE `id` = ?";
...
$sql = "SELECT * FROM `payment` where action='cancelled' LIMIT $offset, $records_per_page";
